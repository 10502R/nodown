...
def test_synthetic_transactions_cover_filter_scenarios():
    ...
    assert {transaction["merchant_status"] for transaction in transactions} >= {"open", "closed"}
...
def test_only_closed_long_term_transaction_is_alert_target():
    ...
    assert [row["transaction"]["transaction_id"] for row in targets] == ["TX-1003", "TX-1006"]
